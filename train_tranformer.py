...
discriminator = Discriminator(input_channels=6).cuda()
...
for epoch in range(EPOCHS):
    for i, batch in enumerate(train_dataloader):
            ...
            for j, val_batch in enumerate(val_dataloader):
                ...
                with torch.no_grad():
                    ...
                    for i in range(Temp):
                        ...
                        # img_pil.show()
                        # 保存图片
                        img_pil.save("./tranformer_Result/"+f"image_transfer_epoch_{epoch}_{i}.png")
                        break
        ...
        if batches_done % SAVE_FREQ == 0:
            torch.save(generator.state_dict(), f"{SAVE_PATH}/generator_transformer_{batches_done}.pth")
